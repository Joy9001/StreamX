...
const MessageThread = ({
  ...
  const [hasCheckedMessages, setHasCheckedMessages] = useState(
    messageCount !== null
  )
  ...
  const { messagesById, loading, sending, error, messageCounts } = useSelector(
  ...
  if (isViewOnly && hasCheckedMessages && localMessageCount === 0) {
    return (
      <button
        disabled
        className='flex cursor-not-allowed items-center rounded-md bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-500 opacity-60'>
  ...
  return (
    <div className='relative'>
      ...
      <button
        ...
        className={`flex items-center rounded-md ${isOpen ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-600'} px-3 py-1.5 text-sm font-medium transition-all ${isButtonDisabled ? 'cursor-not-allowed opacity-60' : ''}`}
        disabled={isButtonDisabled}
        data-thread-id={threadId}>
        <MessageSquare className='mr-1 h-4 w-4' />
        <span>
          {loading && !hasCheckedMessages && isViewOnly
            ? 'Checking...'
            : getStatusText()}
        </span>
      ...
      {isOpen && (
        <div className='fixed right-4 top-[40%] z-50 w-80 -translate-y-1/2 transform rounded-lg border-2 border-indigo-200 bg-white shadow-2xl ring-4 ring-indigo-50 sm:w-96 md:w-[500px]'>
            ...
            <form
              ...
              <div className='flex'>
                ...
                <button
                  ...
                  className={`flex items-center rounded-r-lg px-3 py-2 ${
