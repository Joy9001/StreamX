...
function Cards() {
  ...
  const filteredData = combinedData.filter((editor) => {
    ...
    const matchesPrice = priceFilter === 'all' || (() => {
      ...
      if (!editor.plans || !Array.isArray(editor.plans) || editor.plans.length === 0) {
        return false;
      }

      // Get all valid prices from basic, standard, and premium plans
      const prices = editor.plans.flatMap(plan => {
        const validPrices = [];
    ...
    const matchesLanguage = languageFilter === 'all' || 
      (editor.languages && Array.isArray(editor.languages) &&
      editor.languages.some(lang => lang && lang.toLowerCase() === languageFilter.toLowerCase()))
      ...
      <div className='my-4 ml-14 flex items-center gap-4'>
        ...
        <div className='relative'>
          <select
            ...
            className='appearance-none input input-bordered rounded border-2 border-solid border-gray-300 p-2 pr-10 bg-white cursor-pointer hover:border-gray-400 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary'
          >
          ...
          <svg
            className='absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 pointer-events-none text-gray-500'
            fill='currentColor'
            viewBox='0 0 20 20'
          >
            <path
