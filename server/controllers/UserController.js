...
export const createUser = async (req, res) => {
	...
	try {
		...
		if (role === 'superuser' && requestingUser.role !== 'superuser') {
			return res
				.status(403)
				.json({ message: 'Authorization not given. Only superusers can create other superusers.' })
		}

		if (role === 'admin' && requestingUser.role !== 'superuser') {
			return res
				.status(403)
				.json({ message: 'Authorization not given. Only superusers can create other admins.' })
		}
